...
/**
 * The GrundyRecGplusGequalsP class implements a game based on the Grundy number theory.
 * It provides methods to determine winning and losing positions in the game, 
 * play the game, and test various functionalities.
 * 
 * The game involves a set of piles of matches, and players take turns to split a pile into two unequal piles.
 * The player who cannot make a move loses the game.
 * 
 * The class includes methods to:
 * - Determine if a game position is winning or losing.
 * - Play the winning move if it exists.
 * - Normalize and reduce game states.
 * - Test various functionalities and the efficiency of the algorithm.
 * 
 * The main method to start the game is `principal()`, which runs a series of tests and then starts the game loop.
 * 
 * The class uses recursive methods to determine winning and losing positions and maintains lists of known 
 * winning and losing positions to optimize the computation.
 * 
...
class GrundyRecGplusGequalsP {
    ...
    /**
     * An array representing the types of piles.
     */
    int[] type = {0, 0, 0, 1, 0, 2, 1, 0, 2, 1, 0, 2, 0, 3, 2, 1, 3, 2, 0, 3, 1, 3, 2, 1, 2, 3, 0, 4, 3, 2, 3, 1, 3, 2, 1, 2, 3, 0, 4, 3, 2, 0, 4, 3, 2, 1, 3, 2, 4}; // tableau des types
    ...
    boolean estPerdante(ArrayList<Integer> jeu) {
        ...
        if (jeu == null) {
            ...
            // Apply Theorem 3.4 and type theory
    ...
    /**
     * Reduces the game state by removing known losing piles and applying type theory.
     * 
    ...
    ArrayList<Integer> reduceJeu(ArrayList<Integer> jeu) {
        ...
        // Apply type theory to remove pairs of winning positions of the same type
        for (int i = 0; i < reduced.size(); i++) {
            for (int j = i + 1; j < reduced.size(); j++) {
                if (type[reduced.get(i)] == type[reduced.get(j)]) {
                    reduced.remove(j);
                    reduced.remove(i);
                    i--;
                    break;
