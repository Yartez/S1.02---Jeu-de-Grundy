...
class GrundyRecBruteEff {
    ...
    void boucleJeu() {
        boolean game = true;
        // Game initialization
        int n = SimpleInput.getInt("Enter the size of the game: ");
        ...
        while (estPossible(jeu)) {
            ...
            if (estPossible(jeu)) {
                int userLigne = SimpleInput.getInt("On which pile to remove matches? ");
                int userMove = SimpleInput.getInt("How many matches to remove? ");
                enlever(jeu, userLigne - 1, userMove);
            }
        }
        
        System.out.println("Game over: " + jeu);
    }
    /**
     * AI's turn method
    ...
    void tourIA(ArrayList<Integer> jeu) {
        ...
        if (!res) {
            ...
            System.out.println("Random move: remove " + allumettes + " matches from pile " + (tas + 1));
